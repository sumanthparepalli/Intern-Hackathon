...
from models import UserAddress, Inventory, Store, CreditScheme
...
"""
    ...
    4. store_coordinates  is a 2d list of stores that contains [[storeId,latitude,longitude],[storeId,latitude,longitude]]
...
def get_inventory_list(store_id_list):
    query_result = db.session.query(Inventory).filter(Inventory.storeId.in_(store_id_list)).all()
    inventory_list = []
    for records in query_result:
...
if __name__ == "__main__":
    inventory_dict = input_for_wrapper(1, 600055)
